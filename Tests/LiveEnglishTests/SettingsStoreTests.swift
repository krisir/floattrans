...
final class SpeechPolicyEvaluatorTests: XCTestCase {
    ...
    func testPauseEventDoesNotSpeakEvenWhenEnabled() {
        XCTAssertFalse(
            evaluator.shouldSpeak(speechEnabled: true, speechTriggers: .all, event: .pause))
        XCTAssertTrue(
            evaluator.shouldSpeak(speechEnabled: true, speechTriggers: .all, event: .completeSentence))
        XCTAssertTrue(
            evaluator.shouldSpeak(speechEnabled: true, speechTriggers: .all, event: .shortcut))
    }
}
